...
import type { AuthFileAuto429Event, AuthFilesResponse } from '@/types/authFile';
import type { OAuthModelAliasEntry } from '@/types';
import { parseTimestampMs } from '@/utils/timestamp';

type StatusError = { status?: number };
type AuthFileStatusResponse = { status: string; disabled: boolean };
export type AuthFileAuto429ProbeResponse = {
  status: string;
  auth_id?: string;
  model?: string;
...
export const authFilesApi = {
  ...
  probeAuto429: (name: string) =>
    apiClient.post<AuthFileAuto429ProbeResponse>('/auth-files/auto-429-probe', { name }),

  getAuto429Events: (name: string) =>
    apiClient.get<AuthFileAuto429EventsResponse>('/auth-files/auto-429-events', {
      params: { name },
    }),
